...
def getOSInfo():
    ...
    # 通过命令行读取系统发行版
    issue = exec_command("cat /etc/issue | awk '{printf(\"%s\", $0)}'")

    # 通过命令行获取时间
    date = exec_command("date | awk '{printf(\"%s\", $0)}'")

    # 通过命令行获取内核版本
    uname = exec_command("uname -r | awk '{printf(\"%s\", $0)}'")

    # 通过命令行获取cpu使用率
    cpuse = exec_command("top -bn 1 | head -5 | awk 'NR ==3' ")
    cpuse = cpuse.decode().split((","))[2]
    cpuse = re.search("\d+(\.\d+)?", cpuse).group()

    # 通过命令行获取内存使用率
    memuse = exec_command("top -bn 1 | head -5 | awk 'NR==4'")
    memt = (re.search("\d+", memuse.decode().split(",")[0])).group()
    memf = (re.search("\d+", memuse.decode().split(",")[1])).group()
    memuse = float(memf) / float(memt)
    # 通过命令行获取硬盘使用率
    disk = exec_command("df -h | awk 'NR==4' ")
    filesystem, size, used, avail, diskuse, mounted = disk.decode().split()
